...
module MusicMaster
  ...
  def self.loadConf
    ...
    if (lConfSource == nil)
      log_err "No MusicMaster configuration file could be found. You can set it by setting MUSICMASTER_CONF_PATH environment variable, or create a musicmaster.conf.rb file either in #{File.dirname(__FILE__)} or the current directory."
    else
      if (File.exists?(lConfigFileName))
        File.open(lConfigFileName, 'r') do |iFile|
          begin
            ...
            log_err "Invalid configuration file #{lConfigFileName} specified in #{lConfSource}: #{$!}"
            $MusicMasterConf = nil
          end
        end
      else
        log_err "Missing file #{lConfigFileName}, specified in #{lConfSource}"
      end
    end
